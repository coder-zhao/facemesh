...
这是一个基于 **MediaPipe Face Landmarker** 的实时相机贴图 Demo，贴图会跟随 FaceMesh（Android/iOS 均为 **多点分片仿射**）。
...
- 贴图（PNG）跟随 mesh：
  ...
  - iOS：`8~20` 点级别的分片三角形仿射跟随（当前与 Android 对齐为 9 点网格）
- Android 可交互控制：
  - 开关贴图显示
  - 滑条调整 mesh 缩放
...
iOS 已与 Android 对齐为 **9 点 + 分片三角形**，每个三角片独立仿射并裁剪绘制。
